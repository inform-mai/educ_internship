...
class VLLMWrapper:
    ...
    async def chat_completion_json(
        ...
        #logger.info(f"LLM json qurry - {messages}")
        message = await self.chat_completion_raw(messages)
        content = (message.get("content") or "").strip()

        try:
            return self._parse_json_lenient(content)
        except json.JSONDecodeError as e:
            raise RuntimeError(f"LLM вернула невалидный JSON:\n{content}") from e
    ...
    def _parse_json_lenient(content: str) -> Dict[str, Any]:
        ...
        # взять только участок от первой { до последней }
        start = content.find("{")
        end = content.rfind("}")
        ...
        # 2. обрезаем лишние закрывающие скобки справа
        fixed = raw
        ...
        # 3. балансировка фигурных скобок
        raw = content[start:]
        balance = 0
        end_idx = None
